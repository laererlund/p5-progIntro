 ...
 * 
 * Modificeret til gymnasiebrug af Simon Moe og Erik Lund
 */
...
function keyPressed() {
    //console.log(key);
    if (keyCode === UP_ARROW) {
        console.log("Der hoppes!");
        for(i=0; i < balls.length; i++){
            balls[i].up();
        }
    }
    else if (keyCode === DOWN_ARROW) {
        console.log("Tasten pil-ned er trykket");
        for(i=0; i < balls.length; i++){
            var farve = color(random(255), random(255), random(255),255);
            balls[i].newCol(farve);            
        } 
    }
}

function mouseClicked(){
    balls.push(new Ball(mouseX, mouseY, random(24, 84), random(-21,12)));
}
...
Vi har nu oprettet et objekt - en bold, som kan hoppe. 
Men det smukke ved objektorienteret programmering er, at vi relativt let kan oprette flere. 
Mange flere. 

For at gøre det, skal vi bruge et andet centralt fænomen indenfor programmering: 
lister - eller på engelsk: arrays.
...
I setup funktionen bliver der nu lavet en enkelt bold - men vi vil lave mange. 
Og vi vil gemme dem i vores array: balls.  
